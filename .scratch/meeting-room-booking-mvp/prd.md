...
직원들이 회의를 잡을 때마다 엑셀 시트, 메신저, 구두 전달을 오가며 어느 회의실이 비어 있는지 확인하다 보니, 정확한 정보 없이 같은 시간에 같은 회의실을 두 사람이 예약하는 일이 잦다. 회사 리소스인 회의실이 예약 정보 분산으로 낭비되고 있다. 핵심 해결 방법은 간단하다 — **모든 예약을 하나의 시스템에 모으고, 예약하려는 순간 자동으로 시간 충돌을 검사**하는 것이다. 이렇게 하면 중복을 원천 차단할 수 있고, 관리자도 전체 회의실 운영 현황을 데이터로 파악해 자원을 최적화할 수 있다.

### 네러티브 구성 요소

| 요소 | 내용 | 근거 |
|------|------|------|
| **대상** | 직원(예약자) — 특히 회의 일정이 잦은 팀장·프로젝트 리더 / 총무·관리자(회의실 운영자) | Problem Statement [명시] |
| **진짜 문제** | 예약 정보 분산 → 정확한 가용 시간 불명 → 중복 예약 발생 → 회의 지연 / 관리자의 리소스 최적화 불가능 | Problem Statement [명시] |
| **맥락·타이밍** | 회사가 AI 해커톤을 통해 내부 운영 문제를 기술로 해결하는 시점. 회의실 공유 리소스 낭비가 경영 개선 우선순위로 부상 | 프로젝트 배경 [추론] |
| **핵심 관점·베팅** | "문제는 기술 부족이 아니라 정보 중앙화 부족이다" — 모든 예약을 DB에 저장하고 예약 시점에 자동으로 충돌 검사 | Implementation Decisions [명시] |
| **해결 방향** | ① 중앙 집중식 DB에 모든 예약 저장 ② 예약 생성 시 자동 시간대 겹침 검사 ③ 실시간 회의실 상태 표시 ④ 관리자용 현황 조회 | Solution [명시] |
| **성공의 모습** | 직원: 30초 내 회의실 확보, 중복 예약 사건 제로 / 관리자: 회의실 사용률 데이터 기반 자원 최적화 | Further Notes - 성공 기준 [명시/추론] |
